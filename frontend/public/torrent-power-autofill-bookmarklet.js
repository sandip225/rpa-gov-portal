...
// This script can be injected into any page without installation

(function() {
    'use strict';
    
    console.log('🤖 Torrent Power Auto-Fill Bookmarklet Loaded');
    
    // Check if we're on the correct page
    if (!window.location.hostname.includes('torrentpower.com') || 
        !window.location.pathname.includes('namechangerequest')) {
        
        // If not on Torrent Power page, redirect there
        if (confirm('🤖 AI Auto-Fill: Redirect to Torrent Power website?')) {
            window.location.href = 'https://connect.torrentpower.com/tplcp/application/namechangerequest';
        }
        return;
    }
    ...
    // If no stored data, prompt user
    if (!userData) {
        const connectionId = prompt('🤖 AI Auto-Fill\n\nEnter your Service/Connection Number:');
        const mobile = prompt('Enter your Mobile Number:');
        const email = prompt('Enter your Email Address:');
        
        if (connectionId && mobile && email) {
            userData = {
            ...
            alert('❌ Auto-fill cancelled. Please provide all required information.');
            return;
        }
    }
